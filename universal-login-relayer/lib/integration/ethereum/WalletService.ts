...
import {ensureNotNull, ensure, RequiredBalanceChecker, computeContractAddress, DeployArgs, getInitializeSigner, DEPLOY_GAS_LIMIT, ETHER_NATIVE_TOKEN} from '@universal-login/commons';
...
class WalletService {
  ...
  async deploy({publicKey, ensName, gasPrice, signature}: DeployArgs) {
    ...
    const args = [publicKey, ...ensArgs as string[], gasPrice, ETHER_NATIVE_TOKEN.address];
