...
import {SignedMessage, DeployArgs, ApplicationInfo, asDeploymentHash, asApplicationInfo, StoredFutureWallet} from '@unilogin/commons';
...
const futureWalletHandling = (futureWalletHandler: FutureWalletHandler) =>
  async (data: {body: Omit<StoredFutureWallet, 'tokenPriceInETH'>}) => {
...
export default (
  ...
  router.post('/future', asyncHandler(
    sanitize({
      body: asObject<Omit<StoredFutureWallet, 'tokenPriceInETH'>>({
