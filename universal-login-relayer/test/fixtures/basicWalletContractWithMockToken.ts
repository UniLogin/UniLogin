...
export async function basicWalletContractWithMockToken(provider: MockProvider, wallets: Wallet[]) {
  ...
  const [, providerWithEns] = await buildEnsService(wallet, 'mylogin.eth');
  ...
  return {wallet, actionKey, provider, providerWithEns, mockToken, master, walletContract, otherWallet, actionWallet};
}
