...
export default async function setupMessageService(knex: Knex) {
  ...
  const gasTokenValidator = {validate: () => {}};
  const messageExecutor = new MessageExecutor(wallet, messageExecutionValidator, messageRepository, minedTransactionHandler, walletContractService, gasTokenValidator as any);
