...
import {calculateMessageHash, GAS_BASE, GAS_FIXED, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION} from '@universal-login/commons';
...
describe('INT: MultiSignatureExecute', async () => {
  ...
  it('Error when not enough tokens', async () => {
    ...
    const signedMessage0 = messageToSignedMessage(message, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
    const signedMessage1 = messageToSignedMessage(message, actionKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
  ...
  it('Error when not enough gas', async () => {
    ...
    const signedMessage0 = messageToSignedMessage(message0, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
    await messageHandler.handleMessage(signedMessage0);
    ...
    const signedMessage1 = messageToSignedMessage(message1, actionKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
  ...
  describe('Transfer', async () => {
    it('successful execution of transfer', async () => {
      ...
      const signedMessage0 = messageToSignedMessage(msg, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
      const signedMessage1 = messageToSignedMessage(msg, actionKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
      await messageHandler.handleMessage(signedMessage0);
      const {messageHash} = await messageHandler.handleMessage(signedMessage1);
      await executionWorker.stopLater();
