...
import {GAS_BASE, GAS_FIXED, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION} from '@universal-login/commons';
...
describe('INT: MessageHandler', async () => {
  ...
  it('Error when not enough tokens', async () => {
    ...
    const signedMessage = messageToSignedMessage({...msg, gasToken: mockToken.address}, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
  ...
  it('Error when not enough gas', async () => {
    ...
    const signedMessage = messageToSignedMessage({...msg, gasLimit}, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
  ...
  describe('Transfer', async () => {
    it('successful execution of transfer', async () => {
      ...
      const signedMessage = messageToSignedMessage(msg, wallet.privateKey, ACTUAL_NETWORK_VERSION, ACTUAL_WALLET_VERSION);
      const {messageHash} = await messageHandler.handleMessage(signedMessage);
      await executionWorker.stopLater();
