...
describe('INT: EstimateGasValidator', async () => {
  ...
  it('throws when not enough gas', async () => {
    ...
    await expect(validator.validate(signedMessage)).to.be.eventually.rejectedWith('out of gas');
  });
  ...
  it('throws when not enough tokens', async () => {
    ...
    await expect(validator.validate(signedMessage)).to.be.rejectedWith('Relayer set gas limit too low');
  });
});
