...
describe('Relayer - MultiSignatureExecute', async () => {
  ...
  it('Error when not enough tokens', async () => {
    ...
    await messageHandler.executeSigned({...message, signature: signature0});
    await expect(messageHandler.executeSigned({...message, signature: signature1}))
  ...
  it('Error when not enough gas', async () => {
    ...
    const signature1 = await calculateMessageSignature(actionKey, message);
    await messageHandler.executeSigned({...message, signature: signature0});
    await expect(messageHandler.executeSigned({...message, signature: signature1})).to.be.eventually.rejectedWith('Not enough gas');
  });
  ...
  describe('Transfer', async () => {
    it('successful execution of transfer', async () => {
      ...
      await messageHandler.executeSigned({...msg, signature: signature0});
      await messageHandler.executeSigned({...msg, signature: signature1});
