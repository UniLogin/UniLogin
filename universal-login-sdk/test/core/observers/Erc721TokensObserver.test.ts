...
describe('INT: Erc721TokensObserver', () => {
  ...
  describe('BalanceObserver', () => {
    ...
    it('1 subscription - tokens changed', async () => {
      ...
      expect(callback.thirdCall.args[0] as IBasicToken[]).to.deep.eq(detailedMockedTokens);
    });
    ...
    it('2 subscription - no change', async () => {
      ...
      expect(callback1.secondCall.args[0] as IBasicToken[]).to.deep.eq(detailedMockedTokens);
      expect(callback2.secondCall.args[0] as IBasicToken[]).to.deep.eq(detailedMockedTokens);
    });
