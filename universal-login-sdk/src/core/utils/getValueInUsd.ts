...
import {safeMultiply, safeMultiplyAndFormatEther, ValueRounder} from '@unilogin/commons';
...
export const getValueInUsd = async (tokenAddress: string, walletService: WalletService, value: string, round = 3) => {
  ...
  return ValueRounder.ceil(safeMultiplyAndFormatEther(utils.bigNumberify(ethValue), etherPriceInUsd), round);
};
