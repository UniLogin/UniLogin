...
contract ENSRegistered {
    ...
    function registerENS(bytes32 _hashLabel, string memory _name, bytes32 _node, bytes32 _hashDomain, ENS ens) internal {
        FIFSRegistrar registrar = FIFSRegistrar(ens.owner(_hashDomain));
        PublicResolver resolver = PublicResolver(ens.resolver(_hashDomain));
