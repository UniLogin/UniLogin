...
export function createProxyDeployWithRefundArgs(keyPair: KeyPair, ensDomainData: EnsDomainData, walletMasterAddress: string, relayerAddress: string, gasPrice: string, name: string = 'name') {
  const walletArgs = setupInitializeWithENSAndRefundArgs({keyPair, ensDomainData, name, relayerAddress, gasPrice});
