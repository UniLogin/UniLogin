...
import {createKeyPair, TEST_GAS_PRICE, ETHER_NATIVE_TOKEN, TEST_TRANSACTION_OVERRIDES} from '@universal-login/commons';
...
describe('UNIT: WalletContract', () => {
  ...
  describe('setRequiredSignatures', async () => {
    ...
    it('fail to change the amount of required signatures if the amount is equal to the actual amount', async () => {
      await expect(walletContract.setRequiredSignatures(1, TEST_TRANSACTION_OVERRIDES)).to.be.revertedWith('Invalid required signature');
    });

    it('fail to change the amount of required signatures if the new amount 0', async () => {
      await expect(walletContract.setRequiredSignatures(0, TEST_TRANSACTION_OVERRIDES)).to.be.revertedWith('Invalid required signature');
    });

    it('fail to change the amount of required signatures if the new amount is higher than keyCount', async () => {
      await expect(walletContract.setRequiredSignatures(await walletContract.keyCount() + 1, TEST_TRANSACTION_OVERRIDES)).to.be.revertedWith('Signatures exceed owned keys number');
    });
  });
});
