...
import {createKeyPair, DEPLOYMENT_REFUND, ETHER_NATIVE_TOKEN, TEST_TRANSACTION_OVERRIDES, signString} from '@universal-login/commons';
...
describe('Counterfactual Factory', () => {
  ...
  it('deploy with the same ens name', async () => {
    ...
    await expect(factoryContract.createContract(newKeyPair.publicKey, initializeData, signature, TEST_TRANSACTION_OVERRIDES)).to.be.revertedWith('Unable to register ENS domain');
  });
  ...
  it('should fail if signed ens name and passed in initialize data are different', async () => {
    ...
    await expect(factoryContract.createContract(keyPair.publicKey, initData, signature, TEST_TRANSACTION_OVERRIDES)).to.be.revertedWith('Invalid signature');
  });
