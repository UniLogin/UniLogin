...
export class PickerIframeInitializer extends IframeInitializerBase {
  ...
  constructor(endpoint: IframeBridgeEndpoint, showTransactionDialogs: boolean, sdkConfig: Partial<SdkConfig>, network?: Network) {
    super(endpoint);
    const upstream = this.getUpstream(network);
    const web3ProviderFactories = setupStrategies(upstream, ['UniLogin', 'Metamask'], showTransactionDialogs, {sdkConfig});
