...
describe('BalanceChecked', () => {
  ...
  it('should throw if balance is too small', async () => {
    ...
    expect(await balanceChangedFor(provider, supportedTokens, TEST_ACCOUNT_ADDRESS)).to.be.undefined;
  });
  ...
  it('should return true if balance is bigger than minimal amount', async () => {
    ...
    expect(await balanceChangedFor(provider, supportedTokens, TEST_ACCOUNT_ADDRESS)).to.eq(ETHER_NATIVE_TOKEN.address);
  });
});
